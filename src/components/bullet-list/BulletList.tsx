...
const BulletList = (props: BulletListType) => {
  const { items, hide, className, style: _style, ...cleanProps } = props;
  ...
  return (
    <div ref={containerRef} style={{ ..._style, height: maxHeight }} className={`${style.container} ${className ?? ''}`} {...cleanProps}>
      <ul>
        {items.map((item) => {
          return (
            <li key={item} style={{ height: '20px' }}>
              {item}
            </li>
          );
        })}
...
const BulletListError = (props: BulletListType) => {
  return <BulletList className={style.error} {...props} />;
};
